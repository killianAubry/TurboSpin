            ...
            AstNode::ApplyGate(gate, args0, args1) => match gate.as_str() {
                ...
                "cz" => {
                    let control = if let Argument::Qubit(identifier, qubit_num) = &args0[0] {
                        ...
                        panic!("there's no argument 0 for the CZ gate")
                    };
                    let target = if let Argument::Qubit(identifier, qubit_num) = &args0[1] {
                        let register = registers.get(&identifier).unwrap();
                        register[(*qubit_num).try_into().unwrap()]
                    } else {
                        panic!("there's no argument 1 for the CZ gate")
                    };
                    // CZ decomposition: H(target) -> CX(control,target) -> H(target)
                    qc.h(target);
                    qc.cx(control, target);
                    qc.h(target);
                }
                "swap" => {
                    let t0 = if let Argument::Qubit(identifier, qubit_num) = &args0[0] {
                        let register = registers.get(&identifier).unwrap();
                        register[(*qubit_num).try_into().unwrap()]
                    } else {
                        panic!("there's no argument 0 for the SWAP gate")
                    };
                    let t1 = if let Argument::Qubit(identifier, qubit_num) = &args0[1] {
                        let register = registers.get(&identifier).unwrap();
                        register[(*qubit_num).try_into().unwrap()]
                    } else {
                        panic!("there's no argument 1 for the SWAP gate")
                    };
                    qc.swap(t0, t1);
                }
                "t" => {
                    for arg in args0.iter() {
                        if let Argument::Qubit(identifier, qubit_num) = arg {
                            let register = registers.get(identifier).unwrap();
                            qc.p(PI / 4.0, register[(*qubit_num).try_into().unwrap()]);
                        }
                    }
                }
                "tdg" => {
                ...
                "s" => {
                    for arg in args0.iter() {
                        if let Argument::Qubit(identifier, qubit_num) = arg {
                            let register = registers.get(identifier).unwrap();
                            qc.p(PI / 2.0, register[(*qubit_num).try_into().unwrap()]);
                        }
                    }
                }
                "sdg" => {
                ...
                "u1" => {
                    let angle = eval_with_context(&args1[0], &context).unwrap();
                    for arg in args0.iter() {
                        if let Argument::Qubit(identifier, qubit_num) = arg {
                            let register = registers.get(identifier).unwrap();
                            qc.p(angle.as_float().unwrap() as Float, register[(*qubit_num).try_into().unwrap()]);
                        }
                    }
                }
                "id" => {}
                g => panic!("unsupported OpenQASM gate `{}`", g),
            },
            AstNode::QReg(_, _) => (),
            other => panic!("unsupported OpenQASM construct: {:?}", other),
        }
    }
    qc
}
